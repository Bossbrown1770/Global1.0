...
// Inject Supabase config into every HTML page
const injectSupabaseConfig = (req, res, next) => {
  if (req.path.endsWith('.html') || req.path === '/') {
    ...
    if (fs.existsSync(filePath)) {
      ...
      const configScript = `<script>window.SUPABASE_URL = '${SUPABASE_URL}'; window.SUPABASE_ANON_KEY = '${SUPABASE_ANON_KEY}';</script>`;
...
app.listen(PORT, () => {
  console.log(`🚀 Global Logistics server running on http://localhost:${PORT}`);
});
